...
class OhMyLightCoordinator:
    ...
    async def _async_setup_listeners(self):
        if self.func_name == FUNC_NAME_LIGHT_SYNC:
            ...
            unsub_callback = async_track_state_change_event(
                self.hass,
                listen_lights,
                self._async_handle_entity_state_change,
            )
            self._unsub_callbacks.append(unsub_callback)
            logger.debug(f"<{self.config_entry.title}> Listening light entity ids: {listen_lights}")
        elif self.func_name == FUNC_NAME_LIGHT_SWITCH_BIND:
            # 监听开关事件和灯实体
            light_entity_ids = self.func_data["light_entity_ids"]
            ...
            self._unsub_callbacks.append(unsub_callback)
            logger.debug(
                f"<{self.config_entry.title}> Listening entity ids: {listen_entities}, is_wireless: {is_wireless}"
            )
        else:
            logger.error(f"Unknown func name {self.func_name} in entry {self.config_entry.title}")
            return

    @callback
    async def _async_handle_entity_state_change(self, event: Event):
        if self.func_name == FUNC_NAME_LIGHT_SYNC:
            await self._async_handle_light_sync(event)
            return
        elif self.func_name == FUNC_NAME_LIGHT_SWITCH_BIND:
            await self._async_handle_light_switch_bind(event)
            return
        else:
            logger.error(f"Unknown func name {self.func_name} in entry {self.config_entry.title}")
            return

    async def _async_handle_light_switch_bind(self, event: Event):
        logger.debug(f"<{self.config_entry.title}> Light switch bind event: {event.as_dict()}")
        light_entity_ids = self.func_data["light_entity_ids"]
        switch_entity_ids = self.func_data["switch_entity_ids"]
    ...
    async def _async_handle_light_sync(self, event: Event):
        logger.debug(f"<{self.config_entry.title}> Light sync event: {event.as_dict()}")
